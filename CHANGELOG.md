...
- PAM module `pam_oidc_device.so` authenticating logins with the OAuth 2.0
...
- Fail-closed behaviour throughout, including a panic guard at the cgo
...
- In-memory test provider and `mock-provider` binary; pamtester integration
  suite running in a stock Debian container.
